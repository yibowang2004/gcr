    ...
    for(k in 1:(m - 1)) {
      for(j in (k + 1):m) {
        W[[i]] <- rbind(W[[i]], c(1, ifelse(cluster_list[[i]]$mom[j] == cluster_list[[i]]$mom[k], 1, 0),
                                  ...
                                  ifelse(cluster_list[[i]]$husEmpl[j] == cluster_list[[i]]$husEmpl[k] && cluster_list[[i]]$husEmpl[k] == 0, 1, 0),
                                  ifelse(cluster_list[[i]]$husEmpl[j] == cluster_list[[i]]$husEmpl[k] && cluster_list[[i]]$husEmpl[k] == 1, 1, 0),
                                  ifelse(cluster_list[[i]]$husEmpl[j] == cluster_list[[i]]$husEmpl[k] && cluster_list[[i]]$husEmpl[k] == 2, 1, 0),
                                  ifelse(cluster_list[[i]]$husEmpl[j] == cluster_list[[i]]$husEmpl[k] && cluster_list[[i]]$husEmpl[k] == 3, 1, 0),
                                  ifelse(cluster_list[[i]]$husEmpl[j] == cluster_list[[i]]$husEmpl[k] && cluster_list[[i]]$husEmpl[k] == 4, 1, 0)))
      }
    }
  }
}
...
```{r}
library(gcr)
res5_3 <- gcr(Y, X, W, c(res5_2$alpha, -0.01, -0.01, -0.05, 0.3, 0.06, 0.03, 0.01, 0.06), res5_2$beta, 1, family = "Binomial", verbose = T, accelerate = T, lambda = 0.7)
```